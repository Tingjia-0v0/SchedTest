...
func WaitForSSH(timeout time.Duration, opts SSHOptions, stop chan error, debug bool) error {
	pwd := "pwd"
	...
	for {
		...
		args := append(SSHArgs(debug, opts.Key, opts.Port), opts.User+"@"+opts.Addr, pwd)
		if debug {
