...
func (r *randGen) enabledCtors(kind string) []ResourceCtor {
	var ret []ResourceCtor
	ret = append(ret, r.target.resourceCtors[kind]...)
	return ret
}
...
func (r *randGen) generateCall(s *state) []*Call {
	meta := r.target.Syscalls[r.Intn(len(r.target.Syscalls))]
