...
import (
	...
	"schedtest/pkg/cover"
	"schedtest/pkg/cover/backend"
	"schedtest/pkg/flatrpc"
	"schedtest/pkg/fuzzer/queue"
	"schedtest/pkg/log"
	"schedtest/pkg/mgrconfig"
	"schedtest/pkg/report"
	"schedtest/pkg/signal"
	"schedtest/pkg/stat"
	"schedtest/pkg/vminfo"
	"schedtest/prog"
...
type Server interface {
	...
	CreateInstance(id int, injectExec chan<- bool, updInfo vm.UpdateInfo) chan error
...
func (serv *server) runCheck(ctx context.Context, info *handshakeResult) error {
	...
	// the queue source is the source of the queue.Request
...
func (serv *server) CreateInstance(id int, injectExec chan<- bool, updInfo vm.UpdateInfo) chan error {
	runner := &Runner{
		id:            id,
...
func (serv *server) StopFuzzing(id int) {
	...
	if runner.updInfo != nil {
		runner.updInfo(func(info *vm.Info) {
