...
func main() {
	...
	for what := range unsupported {
		tool.Failf("%v is unsupported on amd64 (typo?)", what)
	}
...
func newSyscallData(target *targets.Target, sc *prog.Syscall, attrs []uint64) SyscallData {
	...
	return SyscallData{
		...
		CallName: callName,
		NR:       int32(sc.NR),
		NeedCall: !target.HasCallNumber(sc.CallName) &&
...
// nolint: lll
// registerTempl is used to generate register.go file, will init target for each os and arch
